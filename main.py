...
def print_menu():
    ...
    print(f"{Fore.GREEN}88{Style.RESET_ALL}. {EMOJI['BACKUP']} {translator.get('menu.batch_register', default='批量注册账号')}")
    print(f"{Fore.GREEN}99{Style.RESET_ALL}. {EMOJI['ARROW']} {translator.get('menu.quick_select', default='快速选择账号')}")
    print(f"{Fore.YELLOW}{'─' * 40}{Style.RESET_ALL}")
...
def main():
    ...
    while True:
        try:
            choice = input(f"\n{EMOJI['ARROW']} {Fore.CYAN}{translator.get('menu.input_choice', choices='0-10, 88, 99')}: {Style.RESET_ALL}")
            ...
            if choice == "0":
                ...
                input(f"{translator.get('menu.press_any_key')}")
                return
            else:
                import quit_cursor
                quit_cursor.quit_cursor(translator)
